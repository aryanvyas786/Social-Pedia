...
commentController.post("/", verifyToken, async (req, res) => {
  try {
    const createdComment = await (
      await Comment.create({ ...req.body, user: req.user.id })
    ).populate("user", "-password");
...
commentController.put("/:commentId", verifyToken, async (req, res) => {
  try {
    ...
    if (comment.user.toString() === req.user.id.toString()) {
      ...
      return res
        .status(403)
        .json({ msg: "You can update only your own comments" });
    }
  } catch (error) {
    return res.status(500).json(error.message);
  }
  ...
  async (req, res) => {
    try {
      ...
      if (!comment.likes.includes(currentUserId)) {
        ...
        return res
          .status(200)
          .json({ msg: "Comment has been successfully liked!" });
      } else {
        comment.likes = comment.likes.filter((id) => id !== currentUserId);
        await comment.save();
        return res
          .status(200)
          .json({ msg: "Comment has been successfully unliiked" });
      }
    } catch (error) {
      return res.status(500).json(error.message);
    }
