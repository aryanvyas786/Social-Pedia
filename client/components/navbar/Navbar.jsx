...
const Navbar = () => {
  ...
  useEffect(() => {
    if (searchText) {
      setFilteredUsers(
        allUsers.filter((user) => user.username.includes(searchText))
      );
    } else {
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/user/updateUser/${user._id}`,
        {
          headers: {
          ...
          {showMobileNav && (
            <div className={classes.navigation}>
              ...
              <div className={classes.right}>
                ...
                >
                  <AiOutlineUser
                    onClick={() => navigate(`/profileDetail/${user._id}`)}
                  />
                  <AiOutlineLogout onClick={handleLogout} />
                </div>
                <img
